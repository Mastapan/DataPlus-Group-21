...
preProcess <- function(park, ride, windowSize,startDate = "11/01/2009", endDate = "12/31/2016"){
  ...
  # filter out time inconsistencies entries# 
  data$convertedDate <- paste0(sapply(data$SMONTH, singleToDoubleDigit), "/", 
                               sapply(data$SDAY, singleToDoubleDigit), "/", 
                               sapply((data$SYEAR-2000), singleToDoubleDigit))
  ...
  unqDays <- unique(apply(data[,c("SMONTH", "SDAY", "SYEAR")], 1,
                          paste, collapse = "/"))
  ...
  data <- data[order(data$SYEAR, data$SMONTH, data$SDAY, 
                     data$totmin, decreasing = FALSE),]
  return(list(data = data, unqDays = unqDays, numUnqDays = numUnqDays, 
              numWindowsPerDay = numWindowsPerDay, windowSize = windowSize))
}
...
findStartTimePerDay <- function(infoList){
  ...
  for (i in 1: nrow(data)){
    ...
    startTimeList[[as.character(correctDay)]] = startTime
  }
  return(startTimeList)
}

windowGenerationPerPeriod <- function(infoList){
  startTimeList <- findStartTimePerDay(infoList)
  data <- infoList$data
  ...
  for(i in 1:(numWindowsPerDay-1)){
    windowEntries[i] <- paste0(totalMinToHours((i-1)*windowSize), "-", 
                               totalMinToHours(i*windowSize))
  }
  windowEntries[numWindowsPerDay] <- paste0(totalMinToHours((numWindowsPerDay-1)*windowSize),
                                            "-", totalMinToHours(1440))
  
 
  # new table that we want to produce # 
  results <- data.frame(date = rep(strptime(unqDays, "%m/%d/%Y", tz = "EST"), each = numWindowsPerDay), 
                        window = rep.int(windowEntries, numUnqDays))
  ...
  # the third and fourth display the start and end of the time range respectively in minutes from Midnight# 
  ...
  windowGenerationPerDay <- function(index){
    ...
    for (i in 1:numWindowsPerDay){
      ...
      while (currentPosition <= nrow(data.wanted) & 
             data.wanted[currentPosition, "totmin"] >= results$windowStartSinceMidnight[i] & 
             data.wanted[currentPosition, "totmin"] < results$windowEndSinceMidnight[i]){
  ...
  results$windowStartSinceOpen <- (results$windowStartSinceMidnight - rep(unlist(startTimeList), each = numWindowsPerDay))
  results$dayOfWeek <- weekdays(as.Date(results$date,"%Y-%m-%d"))
...
mk03.process <- preProcess(park = "MK", window=15,ride = 3, "01/01/2014", "12/31/2014")
mk04.process <- preProcess(park = "MK", window=15,ride = 4, "01/01/2014", "12/31/2014")
